...
if (-not $Updated) {
  ...
  elseif ($r -eq 'same') {
    $dv = Deployed-Version
    if ($dv -ne (App-Version)) {
      Warn 'Последняя версия скачана, но на сайт ещё не выложена (прошлое обновление не дошло до конца) — выкладываю.'
      Say "  Сейчас на сайте: $(if ($dv) { $dv } else { 'неизвестно' })"
    } else {
      Ok 'У вас уже последняя версия, и она выложена на сайт.'
